...
add_action( 'tutor_enrollment/after/cancel', 'vca_prevent_enrollment_cancellation_if_paid', 10, 1 );
add_action( 'tutor_enrollment/after/pending', 'vca_prevent_enrollment_cancellation_if_paid', 10, 1 );

function vca_prevent_enrollment_cancellation_if_paid( $enrollment_id ) {
    global $wpdb;
...
// Version compatibility: extend Tutor Utils class to restore removed methods get_enrolments and get_total_enrolments
class VC_Tutor_Utils_Extended extends \TUTOR\Utils {
...
if ( class_exists( '\TUTOR\Utils' ) ) {
    $GLOBALS['tutor_utils_object'] = new VC_Tutor_Utils_Extended();
}